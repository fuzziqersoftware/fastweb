...
void MemoryResourceManager::add_directory(const string& directory, int gzip_compress_level) {
  this->add_directory_recursive(directory, directory, phosg::stat(directory).st_mtime, gzip_compress_level);
}

shared_ptr<const ResourceManagerBase::Resource> MemoryResourceManager::get_resource(const string& name) const {
...
void MemoryResourceManager::add_directory_recursive(
  ...
  for (const string& item : phosg::list_directory(full_path)) {
    ...
    if (type == S_IFDIR) {
      this->add_directory_recursive(base_path, item_full_path, st.st_mtime, gzip_compress_level);
      ...
      if (existing_file_it == this->path_to_resource.end()) {
        ...
        shared_ptr<Resource> res(new Resource(
            phosg::load_file(item_full_path),
            st.st_mtime,
            mime_type_for_filename(item),
            gzip_compress_level));
        ...
        this->name_to_resource.emplace(item_relative_path, existing_file_it->second);
      }
